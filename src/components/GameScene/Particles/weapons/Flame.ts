...
export function createFlame(
    ...
    const mainBeam = new ParticleSystem({
        duration: 1,
        looping: true,
        startLife: new IntervalValue(0.25, 0.35),
        startSpeed: new ConstantValue(1),
        startSize: new ConstantValue(1.25),
        startColor: new ConstantColor(new Vector4(0.85, 0.41, 0.26, 0.5)),
        worldSpace: true,
    ...
    mainBeam.addBehavior(
        new SizeOverLife(
            new PiecewiseBezier([[new Bezier(1, 0.45, 0.35, 0.25), 0]])
        )
    );
    ...
    glowBeam.addBehavior(
        new SizeOverLife(
            new PiecewiseBezier([[new Bezier(1, 0.95, 0.75, 0.3), 0]])
        )
    );
